...
const SelectedOrderPage = (props: any) => {
  ...
  if (location == null) {
    console.error('NOTE(jonathanng) - If state is null, either 1) the Link props.state you clicked has a null request, 2) The props.state for Link is null or 3) you are trying to go to this page without a Link. However you should have access to the url in the format orders/:id. Code needs to be written to get the request by id from the db when state is null.')
    console.error('more info here: https://ui.dev/react-router-pass-props-to-link/#:~:text=To%20do%20this%20with%20React,Route%20that%20is%20being%20rendered.&text=To%20do%20this%20(and%20to,the%20user%20is%20coming%20from%20.')
  } else {
          ...
          {upload && !done && (
            <div style={{ display: 'flex', marginBottom: 20 }}>
              <PrimaryButton onPress={async () => {
                const verificationResult = await axios.post(`${API_URL}/request/finish`, { id: location.state.request.id }).catch((e) => {
                  console.log(e);
                });
                setDone(true)
